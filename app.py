...
if __name__ == "__main__":  
    ...
    reload_enabled = os.getenv("SINOTRUST_RELOAD", "0") == "1"  
    # Stage 3G defaults to a single stable process. Opt into reload explicitly  
    # during development; automatic reload can interrupt long media requests.  
  
    if reload_enabled and module_name.isidentifier():  
        uvicorn.run(  
            f"{module_name}:app",  
            host=os.getenv("SINOTRUST_HOST", "127.0.0.1"),  
            port=int(os.getenv("SINOTRUST_PORT", "8000")),  
            reload=True,  
            log_level="info",  
            server_header=False,  
        )  
