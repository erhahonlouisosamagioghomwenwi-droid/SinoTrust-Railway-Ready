...
async def level11_security_observability_middleware(request: Request, call_next):
    ...
    if APP_ENV == "production" and ENFORCE_PUBLIC_HOST and request.url.path not in {"/healthz", "/readyz"}:
    ...
    if APP_ENV == "production" and FORCE_HTTPS and request.url.path != "/healthz":
