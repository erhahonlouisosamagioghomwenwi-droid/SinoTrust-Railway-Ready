...
# "core" keeps the web service online while external providers are progressively
# connected; provider-dependent operations remain fail-closed at their own gates.
# "strict" restores the full production readiness gate for final launch.
...
Full operational readiness remains strict and reports every missing provider.
Core readiness is intentionally narrower so Railway can keep the application
online while optional/external integrations are being connected.
"""
strict = APP_ENV == "production"
...
run_network_probes=bool(
strict and READINESS_NETWORK_PROBES and SINOTRUST_STARTUP_POLICY == "strict"
),
)

core_checks = {
"database": bool(base_checks.get("database", False)),
"public_url_https": bool(base_checks.get("public_url_https", True)),
"public_host_configuration": bool(base_checks.get("public_host_configuration", True)),
"supported_region": bool(base_checks.get("supported_region", False)),
}
...
"startup_gate_required": bool(
APP_ENV == "production" and IS_LIVE_MODE and SINOTRUST_STARTUP_POLICY == "strict"
),
"region": DEPLOYMENT_REGION,
"instance": SERVICE_INSTANCE,
"database_runtime": DATABASE_ENGINE,
...
"core_readiness": bool(core_ready),
...
"ready": full_ready,
"operational_status": "ready" if full_ready else "degraded",
}
...
probe_ready = report["ready"] if SINOTRUST_STARTUP_POLICY == "strict" else report["core_readiness"]
payload = {
...
return JSONResponse(payload, status_code=200 if probe_ready else 503)
...
"startup_policy": SINOTRUST_STARTUP_POLICY,
"ready": report["ready"],
"core_ready": report["core_readiness"],
"operational_status": report["operational_status"],
"level4_code_gate": code_gate["ok"],
"missing_required_environment": report["missing_required_environment"],
}, ensure_ascii=False))

# Code integrity is always fail-closed. External-provider completeness is
# enforced only in strict mode; provider-specific operations keep their own
# fail-closed guards while the core web service remains deployable.
if not code_gate["ok"]:
raise RuntimeError("SinoTrust Level 4G code gate failed: " + ", ".join(code_gate["failures"]))

if APP_ENV == "production" and IS_LIVE_MODE and SINOTRUST_STARTUP_POLICY == "strict" and not report["ready"]:
raise RuntimeError(
...
if APP_ENV == "production" and not report["ready"]:
logger.warning(json.dumps({
"event": "production_1_0_degraded_startup",
"startup_policy": SINOTRUST_STARTUP_POLICY,
"core_ready": report["core_readiness"],
"missing_required_environment": report["missing_required_environment"],
}, ensure_ascii=False))
