    ...
    if APP_ENV == "production" and ENFORCE_PUBLIC_HOST and request.url.path not in {"/healthz", "/readyz", "/metrics"}:  
        ...
        if host not in ALLOWED_PUBLIC_HOSTS:  
            return _apply_security_headers(  
                request,  
                JSONResponse({"error":"invalid_host","request_id":request_id}, status_code=400),  
            )  
  
    if APP_ENV == "production" and FORCE_HTTPS and request.url.path not in {"/healthz", "/metrics"}:  
...
def production_1_0_status():  
    ...
    return {  
        ...
        "operating_mode": OPERATING_MODE,  
        "live_mode": IS_LIVE_MODE,  
        "startup_gate_required": bool(APP_ENV == "production" and IS_LIVE_MODE),  
        "region": DEPLOYMENT_REGION,  
        "instance": SERVICE_INSTANCE,  
        "database_runtime": DATABASE_ENGINE,  
        "database_target": DATABASE_TARGET_ENGINE,  
...
async def production_1_0_startup_gate():  
    ...
    # Development/demo remains easy to run. Production + LIVE is always fail-closed:  
    # operators cannot disable this gate with an environment flag.  
    enforce = (  
        (APP_ENV == "production" and IS_LIVE_MODE)  
        or os.getenv("SINOTRUST_ENFORCE_PRODUCTION_READINESS", "0") == "1"  
    )  
    if not code_gate["ok"]:  
